...
export type Study = {
  ...
  live: boolean;
  sections: { n: string; title: string; pull?: string; body: string[] }[];
    ...
    sections: [
      {
        ...
        body: [
          'An investigation is a field job. Someone has to stand at an address, speak to a neighbour, photograph a house and form a judgement about whether a borrower is who and where they say they are. Under S.P. Madrid’s operations, 978 of them work across the country at the same time.',
          'The moment the investigator leaves the office, the operation goes blind. It stays blind until paperwork comes back, sometimes days later, and what comes back is a description of the visit rather than the visit itself.',
          'Blindness of that size does not stay a management inconvenience. It becomes a credibility problem, because a recorded visit and an actual visit look identical on paper. The operation could not reliably separate a slow route from a route that was never driven.',
          'That puts fraud exposure on the lending institution and leaves honest investigators under the same suspicion as everyone else.',
        ],
      },
      {
        n: '02',
        ...
        body: [
          'The obvious answer is supervision, and it does not survive contact with the numbers. Supervisors scale linearly with headcount, cost linearly with headcount, and produce a report of a report rather than evidence. The layer meant to solve the blindness becomes another place information gets lost.',
          'The second problem is where proof has to come from. Evidence of a visit is only worth anything if it is captured at the moment of the visit, and that is exactly the moment nobody is watching and the investigator has every reason to keep moving.',
          'Any verification that costs the fieldman extra time gets worked around, and a verification that gets worked around is worse than none, because it looks like assurance.',
          'So the requirement was narrow and awkward: make 978 people legible to the office in real time, without inserting a layer of people between the field and the decision, and make the proof a by-product of doing the job.',
        ],
      },
      {
        n: '03',
        ...
        body: [
          'The app came first. Nothing downstream can be trusted if the input is a description written after the fact, so the work started at the point of capture. The design constraint was that filling the form had to be faster than writing the report by hand. Verification came along as a consequence of that rather than as an addition to it, which is the only version of it that survives contact with a working fieldman.',
          'Then the office. With capture structured, the command center could be built around the record the app already produced. Building the office view first would have produced a dashboard fed by whatever the field happened to send. Building it second meant it could be fed by a record with a known shape.',
          'Then the configurable shape. Different institutions want the investigation in different shapes: different fields, different criteria, different report formats. Every client-specific accommodation becomes a variant, and every variant becomes a special case someone has to remember. Seen from the client’s end it is a flexibility problem; seen from the operation’s end it is a standardisation problem. They are the same problem, and one answer closes both: make the shape itself configurable, and push that logic back into the form and rating builders, so a new investigation type is defined rather than released.',
          'Then integrity. Anomaly detection was last because it needs a baseline, and the baseline is the accumulated history of what real visits look like. Calibration ran roughly two to three months.',
          'Then sideways. The same infrastructure carried the adjacent work: demand letters, skips and collect, litigation, TeleCI.',
        ],
      },
      {
        n: '04',
